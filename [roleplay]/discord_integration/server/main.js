...
function GetDiscordRoles(discordId, callback) {
    ...
    PerformHttpRequest(`https://discord.com/api/guilds/${GUILD_ID}/members/${discordId}`, (err, response, headers) => {
        console.log(`[DEBUG] HTTP request completed with status code: ${err}`);
        if (err === 200) {
            const data = JSON.parse(response);
            console.log(`[DEBUG] Successfully fetched roles for Discord ID: ${discordId}`);
            callback(data.roles);
        } else {
            console.log(`[ERROR] Failed to fetch roles for Discord ID: ${discordId} (Error: ${err})`);
            callback(null);
        }
    }, 'GET', '', { "Authorization": `Bot ${DISCORD_BOT_TOKEN}` });
}
...
function GetDiscordId(playerId) {
    ...
    for (const id of identifiers) {
        if (id.includes("discord:")) {
            ...
            console.log(`[DEBUG] Found Discord ID: ${discordId} for player ID: ${playerId}`);
            return discordId;
        }
    }
...
function CheckPlayerRole(playerId, callback) {
    ...
    if (discordId) {
        GetDiscordRoles(discordId, (roles) => {
            if (roles) {
                console.log(`[DEBUG] Roles fetched for player ID: ${playerId}: ${roles.join(', ')}`);
                for (const role of roles) {
                    if (role === REQUIRED_ROLE_ID) {
                        console.log(`[DEBUG] Player ID: ${playerId} has the required role.`);
                        callback(true);
                        return;
                    }
                }
            }
            console.log(`[DEBUG] Player ID: ${playerId} does not have the required role.`);
            callback(false);
        });
    } else {
        console.log(`[DEBUG] No Discord ID found for player ID: ${playerId}`);
